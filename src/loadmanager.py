...
class Loadmanager(object):
    ...
    def do_partition(self):
        # Offests and Ranges.
        trajectory_ID_offsets = np.insert(np.cumsum(trajectory_lengths)[:-1],0,0)
        trajectory_ID_ranges = zip(trajectory_ID_offsets, np.cumsum(trajectory_lengths))
        ...
        cost_sums, transposed_partitions = self._divide_work(mpi_size, costs, items)
        # partitions :: [[(a0,b0,..)]]
    ...
    def get_myworkshare(self):
        ...
        logger.debug("Fetching workshare for %s",self.my_rank)
...
# ----------------------------------------------------------------
# Parts for using your own schedules.

def run_schedule(schedule, my_rank, mpi_send_lambda, mpi_recv_lambda):
    ...
    for node_pair in schedule:
        ...
        if my_rank == send_rank:
            ...
            print "[parallel] rank %s sending to %s "%(
                                            my_rank, dest_rank )

        elif my_rank == dest_rank:
            recv_object = mpi_recv_lambda(send_rank)
            print "[parallel] rank %s recieving from %s "%(
                                            my_rank, send_rank )
...
# ----------------------------------------------------------------
# Implementations of schedules.

def make_ring_schedule(size, k): # need to rename function and 'k'
    """
    Expected Types:
    - k           :: Integer
    - size        :: Integer
